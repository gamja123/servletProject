<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    int fileCount =0;
    if(request.getParameter("addcnt") != null){//파일 존재
...
function addFile(formName){
	if(formName.addcnt.value == ""){
		alert("입력할 파일 개수를 입력하고 확인 버튼을 눌러 주세요");
	...
	for(idx=0;idx<formName.elements.length;idx++){
		if(formName.elements[idx].type=="file"){
			if(formName.elements[idx].value==""){
				var message = paramIndex+"번 째 파일 정보가 누락되었습니다.\n업로드할 파일을 선택해주세요";
				alert(message);
...
<%--인덱스의 타입이 파일인지 아닌지 파일일 때만  --%>
<body topmargin="100">
<div align="center">
<font size="2" color="#0000ff">
복수 개의 파일의 업로드를 위하여 파일 개수를 입력한 후 <br>
확인 버튼을 눌러 주세요<br>
입력이 완료 되면 DONE 버튼을 눌러 주세요
</font>
</div><br>
...
<table width="75%" border="1" align="center" cellpadding="1" cellspacing="1" bordercolor="#660000" bgcolor="#ffff99">
	...
	<tr bgcolor="#ffcc00">
		<td width="40%" >
			...
			<input type="hidden" name="txtAbstarct" value="<%=getParam(request,"abstract")%>">
